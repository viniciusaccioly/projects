...
class coapDtlsServer(object):
  ...
  def __init__(self, host, port):
    ...
    _sock = wrap_server(_sock,keyfile=self.pem['SERVER_KEY'],certfile=self.pem['SERVER_KEY'],ca_certs=self.pem['CA_CERT'])
    _sock.bind(self.host_address)
    _sock.listen(0)
    # Connect the CoAP server to the newly created socket
    self.server = CoAPServer(self.host_address,starting_mid=None,sock=_sock) #cb_ignore_listen_exception=self._cb_ignore_listen_exception)
...
def main():
    #cloud do loffi
    server = coapDtlsServer("0.0.0.0", 5684).server;
    try:
       server.listen(10)
